...
var bareIdentArgs = map[string]string{
	...
	"downloadBlobTo::name":           "SERVER: the same filepath.Base(target) leaf, on the `download %s: %w` error. The sibling `target` row above is MIXED because it carries the user's --out-name template too; this one is only the leaf",
