...
var exitCodeDocs = []ExitCodeDoc{
	...
	},
	{
		...
		Notes: []string{
			...
			"That split is the rule for **every local path the CLI is handed** — a flag's value and a **positional argument** alike, not just images: `civitai generate --input <file>` likewise exits `2` for a path that is not there or is a directory, and `1` when the file is there and the read fails.",
