...
func TestDoUploadHandsBytesToSubmitter(t *testing.T) {
	...
	if err := doUpload(c, fs, []byte("ZIPBYTES"), m, "https://civitai.com/", appapi.Provenance{}, submitInvocation{}); err != nil {
...
func TestDoUploadSuccessOutput(t *testing.T) {
	...
	if err := doUpload(c, fs, []byte("ZIP"), m, "https://civitai.com/", appapi.Provenance{}, submitInvocation{}); err != nil {
		t.Fatalf("doUpload: %v", err)
	}
	s := out.String()
