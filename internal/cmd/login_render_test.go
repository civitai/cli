...
func TestRenderDeviceInstructionsEmptyURIFallback(t *testing.T) {
	da := &appapi.DeviceAuth{UserCode: "ABCD-1234"}
	var buf bytes.Buffer
	renderDeviceInstructions(&buf, da, "https://civitai.com")
	out := buf.String()
...
func TestDeviceVerificationURIFallback(t *testing.T) {
	if got := deviceVerificationURI(&appapi.DeviceAuth{VerificationURI: "https://x/y"}, "https://b"); got != "https://x/y" {
		t.Errorf("expected the server-supplied URI, got %q", got)
	}
	if got := deviceVerificationURI(&appapi.DeviceAuth{}, "https://b"); got != "https://b/login/oauth/device" {
