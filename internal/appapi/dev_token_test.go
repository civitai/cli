...
func TestMintDevTokenSendsBearerAndBody(t *testing.T) {
	...
	tok, err := c.MintDevToken(context.Background(), "my-block", []string{"ai:write:budgeted"}, nil, true)
	if err != nil {
...
func TestMintDevTokenOmitsEmptyScopes(t *testing.T) {
	...
	c := New(srv.URL, "tok", "")
	if _, err := c.MintDevToken(context.Background(), "my-block", nil, nil, false); err != nil {
		t.Fatalf("MintDevToken: %v", err)
	}
	if hasScopesKey {
	...
	if _, err := c.MintDevToken(context.Background(), "my-block", []string{}, nil, false); err != nil {
		t.Fatalf("MintDevToken: %v", err)
	}
	if hasScopesKey {
