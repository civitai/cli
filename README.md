...
| `have diverged` / `canonical repository have diverged` | The **remote-commit sync**: your clone and the app's canonical repository have both gained commits, so the fast-forward that keeps the website's manifest edits out of harm's way cannot run. Reconcile them, or pass `--no-pull`. Exit `1`. | [Exit code 1](#exit-code-1) |
