...
| `Civitai returned HTTP` | A throttle that carried `Retry-After` and survived every read retry. Exits **`5`**, not `6` — once the retries are exhausted this is a service-availability failure, and `5` is the code to retry on. You will **not** see `rate limited (429)` here. ⚠ The header is checked *before* the message, so a deep-paging-cap 429 that carried `Retry-After` lands here too — exit `5` — rather than on `2`. | [Exit codes](#exit-codes) |
