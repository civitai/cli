...
| `Your repo may be behind what was last released` | The **monotonic-version guard**: `civitai app submit` refused because the manifest version is not strictly above the highest **approved** version of that app, and approving an older (or identical) version replaces the newer live deployment. Bump the manifest — or `civitai app pull` first if the repo is behind what was last released. `--allow-downgrade` submits anyway, for a deliberate rollback. Exit `1`. | [Exit code 1](#exit-code-1) |
