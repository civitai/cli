...
credential problem, unrelated) or a `429`. A failure that sent nothing never
prints the past tense — no usable credential, an unwritable config, a connection
that never opened — so `sent` is a fact about bytes that left this machine and
not a claim about what the CLI meant to send. A refusal that stops the
