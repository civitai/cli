...
written. A project directory the CLI cannot **read** (an unreadable manifest, a
permissions error) likewise produces no object, and exits `1`.
...
| `1` | the object with `"ok": false` for a validation **verdict** — but **nothing** when the failure was a filesystem one |
