...
a file that parses cleanly and registers nothing.
...
exits `1`. **The same is true of a step that fails at write time** — a
destination it will not write, a directory it cannot write into — so a partial
run always reports which step did not happen rather than exiting with an empty
`--json`. **And the same is true of the two instruction files**: `AGENTS.md` and
`CLAUDE.md` each get their own `blocked` row for a broken-symlink destination, a
file that cannot be read, or an `AGENTS.md` carrying two managed blocks. The
three writes are **independent** — one failing does not stop the other two, and
no row ever claims an action for a file nothing attempted.
...
- A **JSONC** config (Zed's `settings.json`, `.vscode/mcp.json`,
  ...
  rather than refused — Zed ships `settings.json` with a comment block, and both
  Zed and VS Code read a trailing comma back happily, so refusing either blamed
  you for a file your editor authored.
...
header holding your actual token is a secret headed for version control. Instead:
...
Verify a setup — this writes nothing:
...
parse failure in their `detail`, `ok: false`, exit `1` — never as an empty
stdout. "Could not look" is not "not registered", and the detail says which. An
...
config that does not parse, a destination it will not write (a broken symlink, for
**any** of the three files), a file it could not write (an unwritable directory),
an `AGENTS.md` that cannot be read or that carries two managed blocks. Each of
those is a `blocked` row in `changes` with `ok: false`, in `--json` and on the
terminal alike. 🔴 **There is exactly one outcome where a step does not happen and
the command still exits `0`: `action: manual`** — `--agent other`, or a
user-scoped agent whose home directory cannot be resolved. There is no file for
this CLI to write in either case, so the servers are printed for you to paste and
the run is a success with a manual step left in it; that is a `changes` row you
...
| `civitai upgrade [--force]` | **Self-update this binary in place** — resolve the latest GitHub release, verify its SHA-256 against `checksums.txt`, and replace the running executable. 🔴 **Not on Windows**: the `.tar.gz` asset it looks for is never published there, so it fails and replaces nothing — upgrade by hand. A **macOS** Homebrew install (the tap is macOS-only) delegates to `brew upgrade` instead; `--force` reinstalls anyway (and self-replaces a Homebrew install). See [Upgrading](#upgrading). |
...
🔴 **`civitai upgrade` does not work on Windows — use the manual path.** It asks
for a `civitai_<version>_windows_<arch>.tar.gz` release asset, but Windows is
published as a **`.zip`** (the release config overrides the archive format for
`goos: windows`), so the lookup can never match: `no release asset … — upgrade
manually from https://github.com/civitai/cli/releases/latest`, and **nothing is
replaced**. Download that release's `.zip` and replace the binary on your
`PATH`, or `npm install -g @civitai/cli@latest`. Linux and macOS are unaffected.
...
| `SHA256 mismatch for` | A download's hash did not match, and the partial file was deleted. Retry — this is integrity checking working, not a bug. The file name in the message is the **uploader's**, so — like every other piece of server text the CLI prints — invisible and terminal-controlling characters are removed from it first. That matters more here than elsewhere: this line is the CLI asserting an integrity *failure*, so an escape sequence in a file name must not be able to rewrite it. The same removal applies to the other download messages that carry server-supplied text; this list names the notable ones and is **not** exhaustive — the download progress line, the same-target overwrite refusal, the `401`/`403`/`404` download errors, the `server returned an unusable filename` refusal (which reaches both stderr and, as the download plan's `target: (unresolved)` note, stdout), the `download …`, `install …`, `create …`, `streaming …` and `finalize …` errors, and the `Saved …` line. Newlines and tabs are removed **too**, on every message in that list: each of them is a single-line surface, so a newline in a file name would let the uploader write a whole line of their own — a forged `Saved … (SHA256 verified)` above a transfer that never finished, say, which is why this matters most on the very line you are reading about. A newline or tab becomes a space, exactly as it does in a table cell (see [What a table cell can contain](#what-a-table-cell-can-contain)). **Two deliberate exceptions, and they do NOT share a reason** — an earlier version of this row said they did, which was true of only the first. (1) The `server returned an unusable filename` refusal renders through `%q`, which **escapes** the newline to a visible `\n`; it is already harmless there, and collapsing it would hide what the server actually sent. (2) Multi-line server text outside this path — a `prompt`, a generation failure reason — is printed **raw** and **indented**, not escaped: its newlines are real, and the indentation is what stops a continuation line reaching column zero. Do not read reason (1) as covering case (2); acting on that would remove the indentation guard, which is the forgery it exists to prevent. | [Download model files](#download-model-files) |
| `checksum mismatch for` | The same, during `civitai upgrade`. The binary was **not** replaced. | [Upgrading](#upgrading) |
| `no release asset` | `civitai upgrade` found the latest release but no archive in it for your OS/arch, so it stopped before downloading anything and **nothing was replaced**. 🔴 **On Windows this is permanent, not a bad release**: the command asks for a `civitai_<version>_windows_<arch>.tar.gz`, and Windows is published as a **`.zip`**, so the lookup can never match on any version. Upgrade by hand — download the `.zip` from the [releases page](https://github.com/civitai/cli/releases/latest) and replace the binary on your `PATH`, or `npm install -g @civitai/cli@latest`. On Linux and macOS the archive normally exists, so this one really does mean the release is missing your platform. | [Upgrading](#upgrading) |
