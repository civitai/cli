...
| `rate limited (429)` | 🔴 **This one message has THREE exit codes — branch on the code, never on the text.** `2` when it is really the deep-paging cap (the message says "too many pages"/"use cursors instead"): the request is structurally doomed, so use `--cursor` rather than `--page`. `5` when a genuine throttle carried `Retry-After` and persisted through all read retries — that is a service-availability failure, not a usage one. `6` for a genuine throttle otherwise; retry with backoff. | [Exit codes](#exit-codes) |
