...
- **A live listing that is still below the publish floor stages, and does not
  ...
  one attaches its image to the revision, leaves the revision **open and
...
| `Your repo may be behind what was last released` / `Resubmitting the version that is already live is almost always an accident` / `That version is approved but not live` | The **monotonic-version guard**: `civitai app submit` refused because the manifest version is not strictly above the highest **approved** version of that app, and approving an older (or identical) version replaces the newer live deployment. Exit `1`; `--allow-downgrade` submits anyway. **The second line tells you which case you are in, and each says only what is actually known.** A **lower** version says your repo may be behind — bump the manifest, or `civitai app pull` first. The **same** version against a deployment that is really live is almost always an accident. The **same** version against an approved row that is **not** live is the retry of a deploy that never landed — a plausible deliberate act, and `--allow-downgrade` resubmits it unchanged. | [Exit code 1](#exit-code-1) |
