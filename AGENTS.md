...
validation finding — the `field` every `--json` consumer groups on; and item 24
covers the CLI-wide exit-code classifier, which every command's published exit
code funnels through. The durable
