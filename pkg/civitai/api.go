...
//
// It exposes the read/download surface (Reader, Downloader, and the typed
// models) plus the Apps submission surface (Submitter, OAuth device flow) so
// external modules can import "github.com/civitai/cli/pkg/civitai" and drive the
// API directly. Build a client with New(baseURL, personalAPIKey, "") for a
// personal-key consumer, or NewWithSource(baseURL, src, "") to supply a custom
// TokenSource (e.g. an OAuth device-login token that refreshes on a 401).
//
